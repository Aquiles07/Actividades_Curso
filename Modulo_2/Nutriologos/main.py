...
def entrenar_modelo(datos):
    ...
    datos['Historial_Medico_Encoded'] = encoder.fit_transform(datos['Historial_Medico'])

    # Combinar variables numéricas y codificadas
    X_numerico = datos[['Edad', 'Peso', 'Altura', 'Historial_Medico_Encoded']]
...
def main():
    ...
    if st.button("Generar Datos y Predecir"):
        ...
        model, X_test, y_test, encoder = entrenar_modelo(datos_simulados)
        y_pred, report = predecir_enfermedades(model, X_test, y_test)

        st.write("Reporte de Clasificación:")
        st.write(report)

        # Visualización de relaciones entre variables usando Altair
        st.subheader("Relación entre Edad, Peso, Altura y Historial Médico:")
        chart = alt.Chart(datos_simulados).mark_circle().encode(
            x='Edad',
            y='Peso',
            size='Altura',
        ...
        # Mostrar la enfermedad predicha para cada caso en el conjunto de prueba
        st.subheader("Enfermedad Predicha para el Conjunto de Prueba:")
        resultados = pd.DataFrame({
            'Edad': X_test['Edad'],
            'Peso': X_test['Peso'],
            'Altura': X_test['Altura'],
            'Historial_Medico': encoder.inverse_transform(X_test['Historial_Medico_Encoded']),
            'Enfermedad_Real': y_test,
            'Enfermedad_Predicha': y_pred
        })
        st.write(resultados)
