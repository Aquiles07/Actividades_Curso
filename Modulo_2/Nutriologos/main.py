...
def main():
    ...
    if st.button("Entrenar Modelo y Predecir"):
        ...
        accuracy, report = predecir_enfermedades(model, datos_simulados)

        st.write(f"Accuracy del modelo: {accuracy:.2f}")
        st.write("Reporte de Clasificación:")
        st.write(report)

        # Mostrar gráficos de barras para comparar variables entre grupos
        st.subheader("Comparación de Variables por Enfermedad Crónica:")
        fig, axes = plt.subplots(1, 3, figsize=(16, 6), sharey=True)
        sns.barplot(x='Enfermedad_Cronica', y='Edad', data=datos_simulados, ax=axes[0])
        axes[0].set_title('Edad')
        sns.barplot(x='Enfermedad_Cronica', y='Peso', data=datos_simulados, ax=axes[1])
        axes[1].set_title('Peso')
        sns.barplot(x='Enfermedad_Cronica', y='Altura', data=datos_simulados, ax=axes[2])
        axes[2].set_title('Altura')
        st.pyplot(fig)
